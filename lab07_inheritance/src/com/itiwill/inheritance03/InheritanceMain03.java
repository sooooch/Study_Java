...
public class InheritanceMain03 {
	...
	public static void main(String[] args) {
		...
		car3.drive(); // -> 다형성으로 선언되어 있더라도, 오버라이드 되어있는 메서드가 호출됨!
		// car3.charge(100); - > casting을 하지 않으면 SubType의(실제 객체즤) 모든 메서드를 사용할 수는 없음.
		((HybridCar) car3).charge(70); // - > casting을 하면, SubType의 모든 메서드를 사용할 수 있음
        //	 SubType var - new SuperType(); - > 컴파일 오류
        //  HybridCar car = new Car(100, 100); - > 컴파일 오류

		// 다형성의 장점: 코드의 재사용성(re-usability)을 높여줌
		// 예)) 배열 선언, 파라미터 선언 등에서 상위 타입으로 선언한 변수에 하위 타입 객체들을 할당할 수 있기 때문에
		// 코드 재사용성이 높아진다.
	...
	public static void test(Car car) {
	  // instanceof 연산자: 객체가 어떤 타입인 지를 반환하는 연산자
		...
		if ( car instanceof HybridCar) {
			System.out.println("하이브리드 자통차 검사...");
