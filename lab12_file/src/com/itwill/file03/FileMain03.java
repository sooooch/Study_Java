...
/*
 * 파일 == FileInputStream ==> 프로그램 BufferedInputStream ==> 프로그램
...
public class FileMain03 {
	...
	public static void main(String[] args) {
		...
		String dest = "data/rating_coppy2.dat";
		...
		try {
			...
			// 리소스 해제: 나중에 만들어 진 스트림 객체를 먼저 close하고,
			// 먼저 만들어진 스트림 객체를 나중에 close해야 함.
			// 스트림 객체들은 생성된 순서의 반대로 close 메서드를 호출해야 함.
			// 가장 마지막에 생성된 스트림 객체만 close하면 나머지는 자동으로 close 된다잉
			try {
				bout.close();
				bin.close();
			} catch (Exception e) {
