 ...
 *  
 *  프로그램<== ObjecInputStream <== FileInoutStream <== 파일
 *  OIS.readObject(): 파일에서 읽은 데이터를 자바 객체로 변환(역직렬화, de-serialize)
 *  
 *  (주의) OIS, OSS에서 읽고 쓰려는 자바 클래스는 반드시 serializable 인터페이스를 구현해야함
 *  
...
public class FileMain04 {
	...
	public static void main(String[] args) {
		...
		try (FileOutputStream out = new FileOutputStream(fileName);
			...
			Product i = new Product(101, "폰", 100_000);
				...
				ObjectInputStream ois = new ObjectInputStream(in);) 
		{
			...
			Product i = (Product)ois.readObject();
